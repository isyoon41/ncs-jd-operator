...
export default async function EditJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: role } = await supabase
    .from("team_roles")
    .select("id, title, intake, teams(name, mission, charter, organizations(name)), jd_versions(version_no, version_major, version_minor, design_snapshot, jd_sections(kind, position, content, metadata))")
    .eq("id", id)
