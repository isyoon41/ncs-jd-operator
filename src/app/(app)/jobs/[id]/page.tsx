...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const storedTeamCharter = record(role.teams?.charter ?? {});
  const teamCharter = Object.keys(designSnapshot).length > 0 ? designSnapshot : storedTeamCharter;
  const roleIntake = record(role.intake);
  const ncsSearchPlan = record(roleIntake.ncsSearchPlan ?? storedTeamCharter.ncsSearchPlan ?? {});
  const appliedTeamContext = record(ncsSearchPlan.teamContext ?? {});

  const organizationId = role.teams?.organizations?.id ?? null;
  const [{ data: mappingRows }, { data: validationRows }, { data: profile }, { data: latestOrgProfiles }] = await Promise.all([
    supabase
    ...
    organizationId
      ? supabase.from("organization_profiles").select("version_no").eq("organization_id", organizationId).order("version_no", { ascending: false }).limit(1)
      : Promise.resolve({ data: [] }),
  ]);
  const validation = validationRows?.[0] ?? null;
  const latestOrgProfileVersion = latestOrgProfiles?.[0]?.version_no ?? null;
  const hasNewerProfile = Boolean(profile && latestOrgProfileVersion && latestOrgProfileVersion > profile.version_no);
  const isGroundedVersion = Boolean(profile && validation);
  const canRefine = isGroundedVersion && isRefinableJdVersion(version);
  ...
  const companyAnchors = strings(appliedTeamContext.companyAnchors);
  const appliedCapabilities = strings(appliedTeamContext.criticalCapabilities);
  const companySpecificResponsibilities = strings(appliedTeamContext.companySpecificResponsibilities);
  const teamContribution = textField(appliedTeamContext.teamContribution);
  ...
  return (
    <PageContainer wide>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2"><JdPdfDownloadButton data={pdfData} />{canRefine && <Link href={`/jobs/${id}/edit`} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:border-blue-200 hover:text-blue-700"><Pencil className="h-4 w-4" />{nextVersionLabel} 업데이트</Link>}<Link href="/jobs/new" className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white"><Plus className="h-4 w-4" />{isGroundedVersion ? "새 직무설계" : "새 방식으로 재설계"}</Link></div>
        </div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="space-y-5 xl:self-start">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-2 text-blue-600"><Building2 className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.18em]">Company grounding</p></div><h2 className="mt-3 font-black text-slate-900">회사 프로필 v{profile?.version_no ?? "—"}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{profile?.summary ?? "이전 방식으로 생성된 JD라 연결된 회사 프로필이 없습니다."}</p>{hasNewerProfile && <div className="mt-3 rounded-xl bg-blue-50 p-3 text-xs leading-5 text-blue-800">회사 프로필이 v{latestOrgProfileVersion}로 업데이트되었습니다. {canRefine ? <Link href={`/jobs/${id}/edit`} className="font-bold underline">{nextVersionLabel} 업데이트</Link> : <Link href="/jobs/new" className="font-bold underline">새 직무설계</Link>}로 최신 정보를 반영하세요.</div>}{(textField(companyContext.mission) || textField(companyContext.vision) || strings(companyContext.coreValues).length > 0) && <div className="mt-4 border-t border-slate-100 pt-4"><div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-400">미션·비전·핵심가치</p>{companyContext.mvcBasis === "inferred" && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-500">AI 추론</span>}</div>{textField(companyContext.mission) && <p className="mt-2 text-sm leading-6 text-slate-600"><span className="font-bold text-slate-700">미션</span> · {textField(companyContext.mission)}</p>}{textField(companyContext.vision) && <p className="mt-2 text-sm leading-6 text-slate-600"><span className="font-bold text-slate-700">비전</span> · {textField(companyContext.vision)}</p>}{strings(companyContext.coreValues).length > 0 && <div className="mt-2"><BulletList items={strings(companyContext.coreValues)} compact /></div>}</div>}{strings(companyContext.strategicPriorities).length > 0 && <div className="mt-4 border-t border-slate-100 pt-4"><p className="text-xs font-bold text-slate-400">핵심 전략</p><BulletList items={strings(companyContext.strategicPriorities).slice(0, 4)} compact /></div>}</section>
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-2 text-blue-600"><Network className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.18em]">Team design</p></div><h2 className="mt-3 font-black text-slate-900">{role.teams?.name} 설계</h2><p className="mt-3 text-sm leading-6 text-slate-600">{role.teams?.mission}</p>{(teamContribution || companyAnchors.length > 0 || appliedCapabilities.length > 0 || companySpecificResponsibilities.length > 0) && <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50 p-4"><p className="text-xs font-black text-blue-700">이 JD에 적용한 회사 기준점</p>{teamContribution && <p className="mt-2 text-sm leading-6 text-slate-700">{teamContribution}</p>}{companyAnchors.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{companyAnchors.map((anchor) => <span key={anchor} className="rounded-full border border-blue-100 bg-white px-2.5 py-1 text-[11px] font-bold text-blue-700">{anchor}</span>)}</div>}{appliedCapabilities.length > 0 && <div className="mt-4"><p className="text-[11px] font-bold text-slate-500">이 팀에 필요한 핵심 역량</p><BulletList items={appliedCapabilities.slice(0, 4)} compact /></div>}{companySpecificResponsibilities.length > 0 && <div className="mt-4"><p className="text-[11px] font-bold text-slate-500">NCS로 대체하지 않은 회사 고유 책임</p><BulletList items={companySpecificResponsibilities.slice(0, 4)} compact /></div>}</div>}{teamOutputs.length > 0 && <div className="mt-4"><p className="text-xs font-bold text-slate-400">팀 산출물</p><BulletList items={teamOutputs.slice(0, 4)} compact /></div>}{teamResponsibilities.length > 0 && <div className="mt-4"><p className="text-xs font-bold text-slate-400">팀 책임</p><BulletList items={teamResponsibilities.slice(0, 4)} compact /></div>}{suggestedRoles.length > 0 && <div className="mt-4 border-t border-slate-100 pt-4"><p className="text-xs font-bold text-slate-400">권장 역할 포트폴리오</p><div className="mt-3 space-y-2">{suggestedRoles.map((item, index) => <div key={index} className="rounded-xl bg-slate-50 p-3"><p className="text-sm font-bold text-slate-800">{textField(item.title)}</p><p className="mt-1 text-xs leading-5 text-slate-500">{textField(item.purpose)}</p></div>)}</div></div>}</section>
            ...
            {canRefine ? <Link href={`/jobs/${id}/edit`} className="flex items-center justify-between rounded-2xl bg-blue-600 p-5 text-white shadow-lg shadow-blue-600/20"><div><p className="text-sm font-black">현재 직무기술서를 더 업데이트하시겠어요?</p><p className="mt-1 text-xs text-blue-100">선택 입력 후 NCS 재검토로 {nextVersionLabel} 생성</p></div><ArrowRight className="h-5 w-5" /></Link> : isGroundedVersion ? <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><p className="text-sm font-black text-slate-900">새 기준본으로 재설계가 필요합니다</p><p className="mt-1 text-xs leading-5 text-slate-600">현재 버전 체계에서는 반복 업데이트를 지원하지 않는 문서입니다.</p></div> : <Link href="/jobs/new" className="flex items-center justify-between rounded-2xl bg-blue-600 p-5 text-white shadow-lg shadow-blue-600/20"><div><p className="text-sm font-black">회사 맥락부터 다시 설계하세요</p><p className="mt-1 text-xs text-blue-100">새 근거 루프로 검증된 v1.0 생성</p></div><ArrowRight className="h-5 w-5" /></Link>}
          </aside>
        </div>
    </PageContainer>
  );
